package com.bolivar.bioingenieria.app.sigma_bb.client_hexagon.infrastructure.adapters.input.rest.controller.servie_area_controller;

import com.bolivar.bioingenieria.app.sigma_bb.client_hexagon.application.ports.input.ServiceAreaServicePort;
import com.bolivar.bioingenieria.app.sigma_bb.client_hexagon.infrastructure.adapters.input.rest.mapper.ServiceAreaRestMapper;
...
public class ServiceAreaRestAdapter {
    ...
    */
    ...
    @PreAuthorize("hasAuthority('admin.full')") // Solo los usuarios con autoridad 'admin.full' pueden acceder a esta información
    @PostMapping("/v1/api/{headquarterId}")
    public ResponseEntity<?> createServiceArea(
            @Parameter(
                    description = "ID de la sede a la que se asociará el área de servicio",
                    required = true,
                    example = "95f346a1-5b41-404f-a73c-1163afae0e46")
            @PathVariable UUID headquarterId,
            @Valid @RequestBody ServiceAreaCreateRequest serviceAreaCreateRequest) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(serviceAreaRestMapper.toServiceAreaResponseFromUseCase(
                        serviceAreaServicePort.save(
                                headquarterId,
                                serviceAreaRestMapper.toServiceAreaUseCaseRequest(serviceAreaCreateRequest))));
    }
