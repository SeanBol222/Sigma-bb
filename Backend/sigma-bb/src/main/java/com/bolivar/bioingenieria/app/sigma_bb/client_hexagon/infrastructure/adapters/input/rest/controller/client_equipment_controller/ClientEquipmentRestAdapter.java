package com.bolivar.bioingenieria.app.sigma_bb.client_hexagon.infrastructure.adapters.input.rest.controller.client_equipment_controller;

import com.bolivar.bioingenieria.app.sigma_bb.client_hexagon.application.ports.input.ClientEquipmentServicePort;
import com.bolivar.bioingenieria.app.sigma_bb.client_hexagon.infrastructure.adapters.input.rest.mapper.ClientEquipmentRestMapper;
import com.bolivar.bioingenieria.app.sigma_bb.client_hexagon.infrastructure.adapters.input.rest.model.request.client_equipment_request.ClientEquipmentCreateRequest;
...
public class ClientEquipmentRestAdapter {
    ...
    private final ClientEquipmentServicePort clientEquipmentServicePort; // Puerto de servicio para la lógica de negocio relacionada con Client Equipment

    private final ClientEquipmentRestMapper clientEquipmentRestMapper; // Mapper para convertir entre DTOs de REST y modelos de dominio relacionados con Client Equipment
    ...
    @PreAuthorize("hasAuthority('admin.full')") // Solo los usuarios con autoridad 'admin.full' pueden acceder a esta información
    @PostMapping("/v1/api/{serviceAreaId}/{modelId}")
    public ResponseEntity<?> createClientEquipment(
            @Parameter(
                    description = "ID del área de servicio a la que se asociará el equipo",
                    required = true)
            @PathVariable UUID serviceAreaId,
            @Parameter(
                    description = "ID del modelo del equipo a asociar",
                    required = true)
